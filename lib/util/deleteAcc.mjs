...
import removeGroupMessages from './removeGroupMessages.mjs';
...
async function deleteThreadsAndGroups(accID) {
    ...
    const threadsProms = threadsWithoutMaster.map(function (threadID) {
        ...
        updateClient(otherAccID, { action: 'expiredAcc', accID, threadID });
    ...
    const groupRemoveMessages = groupsIsMember.map(({ groupID }) => removeGroupMessages(accID, groupID));
    const allProms = [...threadsProms, ...groupDeleteProms, ...groupLeaveProms, ...groupRemoveMessages];

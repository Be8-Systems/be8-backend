...
async function deleteThreadsAndGroups(accID) {
    ...
    const groups = threadsWithoutMaster.filter((threadID) =>
        threadID.includes('g')
    );
    ...
    const groupDeleteProms = groupsIsAdmin.map(({ groupID }) =>
        deleteGroup(groupID)
    );
    const groupLeaveProms = groupsIsMember.map(({ groupID }) =>
        leaveGroup(accID, groupID)
    );
...
export default async function deleteAcc(accID) {

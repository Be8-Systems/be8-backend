...
async function validRequest(req, res) {
    const versions = await redis.sMembers(
        `groupversions:${req.body.groupID}:${req.body.accID}`
    );
    const keyProms = versions.map((version) =>
        redis.hGetAll(
            `groupkey:${req.body.groupID}:${req.body.accID}:${version}`
        )
    );
    const keys = await Promise.all(keyProms);
    const groupKeys = keys
        .map(function (keyObject, i) {
            if (keyObject) {
                return {

...
async function createThread(accID, threadID, otherAcc, otherAccID) {
    ...
    const lastMessage = await redis.hGetAll(
        `message:${threadID}:${lastMessageID}`
    );
    const { password, ...accObj } = otherAcc; // eslint-disable-line
    const msgStatus = await redis.sMembers(
        `status:${threadID}:${lastMessageID}`
    );
    const status =
        msgStatus.includes(accID) || lastMessage.sender === accID
            ? 'read'
            : 'unread';
    const isImageMessage = lastMessage.type === 'imageMessage';
    const imageText =
        accID === lastMessage.sender ? 'SENTIMAGE' : 'RECEIVEDIMAGE';
...
async function validRequest(req, res) {
    ...
    const masterThread = await createThread(
        accID,
        `${accID}:s1`,
        masterAcc,
        's1'
    ...
    return res
        .status(200)
        .send({ valid: true, threads: [masterThread, ...threads] });
}
